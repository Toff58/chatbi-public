...
当前有 50 条产品级回归评测：测试用例会检查 SQL 是否包含关键模式、是否拒绝危险 SQL、是否命中 RAG 规则、结果行数是否合理、是否暴露中间计算列，以及枚举直答、数据范围、不可支持问题等分支是否按预期处理。日志里也记录 `sql_valid`、`execution_success`、`tool_called`、`result_count`、`latency_ms` 等指标。后续可以继续扩到 100 条，并增加人工标注答案质量。
...
- 将 50 条回归评测扩到 100 条，记录 SQL 准确率、执行成功率、回答可用性和不同风险层级的通过率。
